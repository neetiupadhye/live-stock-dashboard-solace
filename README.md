...
The set of tradeable tickers and their display info live in `solace_common.py`; polling cadence and news behavior live in `publisher.py`:

```python
# solace_common.py
AVAILABLE_TICKERS = ["D05.SI", "O39.SI"]   # any valid Yahoo Finance tickers
TICKER_INFO = {
...
NEWS_ARTICLES_PER_TICKER = 5         # how many articles to keep per ticker per publish
LIVE_FRESHNESS_WINDOW_SECONDS = 180  # how recent a bar must be to count as "live"
```

> The default first ticker (`D05.SI`, DBS Group Holdings on SGX) is what the dashboard's dropdown and the subscriber's initial subscription both default to on startup. Add or remove tickers freely — both the publisher and the dashboard's dropdown pick up the list automatically. A ticker added to `AVAILABLE_TICKERS` without a matching `TICKER_INFO` entry still works; the dashboard falls back to showing its raw symbol with `"—"` for exchange and `"USD"` for currency.

---

## Usage

Publisher and subscriber/dashboard are run as two separate processes. **Start the publisher first** — the subscriber sends its very first backfill request as soon as it connects, and Solace direct messaging doesn't queue that request for a publisher that isn't listening yet.
...
Wait for it to print `Publishing [...] every 15s...` before moving on.
...
> If the dashboard is stuck on "Waiting for data..." after startup, it usually means the subscriber's initial backfill request went out before the publisher was up to hear it. Toggling the ticker dropdown to another stock and back re-sends that request and will unstick it without a full restart.
